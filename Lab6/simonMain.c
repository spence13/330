...
#define SIMON_DISPLAY_TOTAL_SECONDS 120
...
#define SIMON_DISPLAY_TIMER_PERIOD .07//this is the fastest without missing interrupts
#define SIMON_DISPLAY_TIMER_CLOCK_FREQUENCY (XPAR_CPU_CORTEXA9_0_CPU_CLK_FREQ_HZ / 2)
#define SIMON_DISPLAY_TIMER_LOAD_VALUE ((SIMON_DISPLAY_TIMER_PERIOD * SIMON_DISPLAY_TIMER_CLOCK_FREQUENCY) - 1.0)


int main()
{
    //these take too long in any given state
    display_init();//initialize the board
    display_fillScreen(SIMONCONTROL_BLACK);//blank the screen with black
    display_setTextSize(SIMONCONTROL_TEXT_SIZE);//set text size
    ...
    interrupts_setPrivateTimerLoadValue(SIMON_DISPLAY_TIMER_LOAD_VALUE);
    ...
    // interrupt count to determine if you have missed any interrupts.
     while (interrupts_isrInvocationCount() < (SIMON_DISPLAY_TOTAL_SECONDS * privateTimerTicksPerSecond)) {
      if (interrupts_isrFlagGlobal)
      {  // This is a global flag that is set by the timer interrupt handler.
          // Count ticks.
